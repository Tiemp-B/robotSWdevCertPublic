...
"REST API에서는 GET 요청으로도 서버의 자원 상태를 변경(생성·수정·삭제)하는 것이 표준적인 설계다"라는 서술은 오답이다 — REST의 핵심은 오히려 **GET은 조회 전용으로, 상태를 변경하지 않아야 한다**는 원칙이며, 상태 변경은 POST/PUT/DELETE가 담당한다.
