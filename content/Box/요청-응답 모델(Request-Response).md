...
"요청-응답 모델에서는 서버가 클라이언트의 요청 없이도 상태 변화가 있을 때마다 자동으로 데이터를 보내준다"라는 서술은 오답이다 — 그것은 오히려 [[발행-구독 모델(Publish-Subscribe)]]의 특징이며, 요청-응답 모델의 핵심은 **클라이언트의 명시적 요청이 있어야만 서버가 응답**한다는 점이다.
