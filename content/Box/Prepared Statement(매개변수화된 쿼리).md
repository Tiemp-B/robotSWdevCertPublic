...
"Prepared Statement는 사용자 입력값의 특수문자를 제거하거나 이스케이프하는 필터링 기법이다"라는 서술은 오답이다 — Prepared Statement의 핵심은 필터링이 아니라 **쿼리 구조와 데이터를 애초에 분리해 전달**하는 것이며, 입력값을 필터링하는 것과는 다른 접근이다.
